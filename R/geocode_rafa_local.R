...
#' )
#'
#' #df <- geocodebr:::geocode_rafa_local(input_df, address_fields = fields, progress = FALSE)
#' #df
#'
geocode_rafa_local <- function(addresses_table,
  ...
  con <- duckdb::dbConnect(
    duckdb::duckdb(),
    dbdir = 'C:/Users/r1701707/AppData/Local/R/cache/R/geocodebr/test_db/geocodebr_db1.duckdb'
  #  dbdir = 'C:/Users/r1701707/AppData/Local/R/cache/R/geocodebr/test_db/geo.duckdb'
  )
  ...
  for (case in c(1:4,  5:12)) {
    ...
    if (all(relevant_cols %in% names(input_padrao))) {
      ...
      match_fun <- ifelse(case != 44, match_aggregated_cases_local, match_aggregated_cases_weighted)
...
#' #df
#'
geocode_rafa_local2 <- function(addresses_table,
                               address_fields = setup_address_fields(),
                               n_cores = 1,
                               progress = TRUE,
                               cache = TRUE
  ...
  con <- duckdb::dbConnect(
    duckdb::duckdb(),
    dbdir = 'C:/Users/r1701707/AppData/Local/R/cache/R/geocodebr/test_db/geocodebr_db.duckdb'
    #  dbdir = 'C:/Users/r1701707/AppData/Local/R/cache/R/geocodebr/test_db/geo.duckdb'
  )
  ...
  for (case in c(1:4,  5:12)) {
    ...
    if (all(relevant_cols %in% names(input_padrao))) {
      ...
      # select match function
      match_fun <- ifelse(case != 44, match_aggregated_cases_local2, match_aggregated_cases_weighted)
...
geocode_rafa_local_arrow <- function(addresses_table,
  ...
  for (case in c(1:4,  5:12)) {
    ...
    if (all(relevant_cols %in% names(input_padrao))) {
      ...
      # select match function
      match_fun <- ifelse(case != 44, match_aggregated_cases_local_arrow, match_aggregated_cases_weighted)
