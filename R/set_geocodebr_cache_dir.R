#' Set custom cache directory for geocodebr files
#'
#' Set custom directory for caching files from the geocodebr package. If users
...
#'
#' @return A message indicating the directory where geocodebr files are cached.
#'
#' @export
#' @family Cache data
...
#' tempd <- tempdir()
#' set_geocodebr_cache_dir(path = tempd)
#'
#' # back to default path
#' set_geocodebr_cache_dir(path = NULL)

set_geocodebr_cache_dir <- function(path = NULL) {

  if (!is.null(path)) {
    if (!dir.exists(path)) {stop('Directory does not exist.')}
    geocodebr_env$cache_dir <- path
  }
  ...
  message(paste("geocodebr files will be cached at ", geocodebr_env$cache_dir))
